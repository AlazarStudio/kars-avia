...
**Согласование с авиакомпанией** (черновик `type: AIRLINE`): `DRAFT → submitAirlineReportDraft → SUBMITTED → confirmReportDraft → CONFIRMED`. Из `SUBMITTED` диспетчер отзывает свою отправку (`unsubmitAirlineReportDraft`), а авиакомпания возвращает отчёт на доработку с обязательным комментарием (`rejectAirlineReportDraft` → снова `DRAFT`, `rejectedAt` + `airlineComment`). «Сейчас возвращён» = `rejectedAt != null`: повторная отправка гасит отметку, но комментарий оставляет. Открытый редактор живёт в адресе: `/reports?reportdraftid=<id>` — черновик, `/reports?reportid=<savedReportId>` — экранный вид выпущенного (так ведут ссылки из писем бэка); у `SUPERADMIN` такие ссылки перенаправляются на `/reportsV2`.

Раскладка: `ReportsV2.jsx` (контейнер: состояние, запросы, гейты по ролям), `ReportsV2List/`, `ReportDraftsPanel/`, `ReportDraftEditor/` (редактор строк + хук `useReportDraft.js`), `ReportCreateSidebar/`, `ReportRulesSidebar/`. Доменная логика без JSX вынесена в `reportRules.js`, `reportDraftRows.js`, `reportDraftAge.js`, `reportDraftLink.js` (адрес редактора), `reportDraftComment.js` (комментарий АК) — у всех тесты рядом (`node --test src/Components/Blocks/ReportsV2/`).
