...
import classes from "./SupportAnalytics.module.css";
...
function SupportAnalytics() {
  ...
  const { data: airlinesData, refetch: refetchAirlines } = useQuery(
    GET_DISPATCHERS,
    {
      context: {
        headers: {
  ...
  const handleLoadMore = async () => {
    ...
    setLoadingMore(true);
    const res = await refetchAirlines({ pagination: { skip: 0, take: nextTake } });
      ...
      <div className={classes.content}>
        <div className={classes.graphs}>
          <div className={classes.header}>
            ...
            <button className={classes.periodButton} onClick={() => setShowPicker(true)}>Выбрать период</button>
            {showPicker && (
