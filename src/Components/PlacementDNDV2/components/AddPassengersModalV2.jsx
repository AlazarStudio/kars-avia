...
const AddPassengersModalV2 = ({
  ...
  const { data } = useQuery(GET_AIRLINE_USERS, {
    context: {
      headers: {
  ...
  const { data: dataHotelReserveOne } = useQuery(GET_RESERVE_REQUEST_HOTELS, {
    context: {
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  const bronedPersons = requestsHotelReserveOne.flatMap((item) => [
  ...
  const handleSavePerson = async () => {
    ...
    try {
      const reserverAddHotelPassenger = await createRequestPerson({
        variables: {
          input: {
            hotelId: hotelId,
...
export default AddPassengersModalV2;
