...
const Notification = ({ text, status, onClose, time, index }) => {
  ...
  return (
    <div
      className={`${classes.notification} ${classes[status]} ${
        visible ? classes.visible : classes.hidden
      } `}
      style={{ bottom: `calc(${index * 52 + 20}px)` }}
    >
      <span>{text}</span>
