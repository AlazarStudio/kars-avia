...
export default function ManifestUploadField({
  ...
  return (
    <div className={classes.wrapper}>
      ...
      {/* Говорим ровно то, что видно в файле: в колонке «РМ» есть отметки, а своих
          строк у этих пассажиров нет. Как расшифровывается «РМ», в самом манифесте
          не написано — не называем категорию за него. */}
      {parsed?.lapInfants?.count > 0 && (
        <div className={classes.flightWarn}>
          ⚠ В манифесте отмечены ещё {parsed.lapInfants.count}{" "}
          {plural(parsed.lapInfants.count, [
            "пассажир",
            "пассажира",
            "пассажиров",
          ])}{" "}
          без собственных строк — колонка «РМ» заполнена у
          {parsed.lapInfants.carriers?.length
            ? `: ${parsed.lapInfants.carriers.join(", ")}`
            : " части пассажиров"}
          . Проверьте и добавьте вручную, если нужно.
        </div>
      )}
