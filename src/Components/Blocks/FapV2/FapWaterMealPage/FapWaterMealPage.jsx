...
export default function FapWaterMealPage({
  ...
  const handleDelete = async (personIndex) => {
    ...
    try {
      ...
      // Выделение и режим правки адресуют получателей ПОЗИЦИЕЙ в массиве, а
      // удаление её сдвигает. Незакрытая правка после удаления строки выше
      // записывала ФИО, телефон и место СОСЕДУ (у которого оставался свой
      // personId) и переименовывала его запись в реестре — причём форма
      // продолжала показывать редактируемое имя, то есть сигнала не было
      // никакого. Выделение вело себя так же в «Удалить выбранных».
      // Сбрасываем только после успеха: при ошибке ввод терять незачем.
      cancelEdit();
      clearSel();
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      // Выделение здесь чистилось и раньше, а незакрытая правка — нет: её
      // индекс так же съезжал после пакетного удаления.
      cancelEdit();
      clearSel();
      onRefetch?.();
    } catch (err) {
