...
function TransferMessage({
  ...
  const getSender = (message) => {
    if (message.senderUser) return message.senderUser;
    if (message.senderDriver) return message.senderDriver;
    if (message.senderPersonal) return message.senderPersonal;
    return null;
  };
  ...
  const getMessageItems = () => {
    ...
    messages.forEach((message, index) => {
      ...
      const currSenderId = getSenderId(message);
      const isFirstInSenderGroup =
        !isMyMessage(message) &&
        (!prevMessage || prevSenderId !== currSenderId);
                    ...
                    <div className={classes.requestData_message_content}>
                      {isFirstInSenderGroup ? (
                        <div
                          ...
                          style={message?.separator ? { backgroundColor: "#3CBC6726", color: "#3B6C54" } : {}}
                        >
