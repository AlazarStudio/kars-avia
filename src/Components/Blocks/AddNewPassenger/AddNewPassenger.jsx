...
function AddNewPassenger({
  ...
  const handleChange = (e) => {
    ...
    if (name === "passengers" && Number(value) + showChooseHotels > maxCount) {
      setError(
        `Максимальное количество пассажиров - ${maxCount}. Другие гостиницы уже выбрали ${showChooseHotels} пассажиров. Вы можете выбрать не более ${maxCountChoose} пассажиров`
      );
    ...
    try {
      ...
      if (
        e.message.startsWith(
          "This reserve and hotel combination already exists."
        )
      ) {
  ...
  useEffect(() => {
    ...
    if (
      !formData.city &&
      request?.airport?.city &&
      uniqueCities.includes(request.airport.city)
    ) {
...
//   console.log(formData);
  ...
  //   console.log(filteredHotels.filter((i) => i.id === formData.hotel));
    ...
    <Sidebar show={show} sidebarRef={sidebarRef}>
            ...
            <>
              ...
              >
                ...
                {filteredHotels.map((hotel, index) => (
                  ...
                  >
                    {hotel.name}{" "}
                    {addedHotelIds.includes(hotel.id) ? "(уже добавлен)" : ""}
                  </option>
                ))}
              </select> */}
              {user.role !== "HOTELADMIN" && (
                <MUIAutocompleteColor
                  ...
                  // isDisabled={addedHotelIds.includes(hotel.id)}
                  ...
                  value={
                    filteredHotels.filter((i) => i.id === formData.hotel)[0] ||
                    ""
                  }
                  onChange={(event, newValue) => {
                    const matched = filteredHotels.find(
      ...
      <div className={classes.requestButton}>
        <Button onClick={handleSubmit}>
          {user.role == "HOTELADMIN" ? "Добавить" : "Добавить гостиницу"}
        </Button>
      </div>
    </Sidebar>
  );
